// supabase/functions/bulk-resend-tickets/index.ts
// PUP REVO 2026 — Bulk resend ticket confirmation emails
...
// Small delay to avoid hitting rate limits (5 req/sec max)
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: orders, error: fetchError } = await supabase
      ...
      .gte('paid_at', '2026-05-30 06:19:56')

    if (fetchError) throw fetchError

    console.log(`Found ${orders.length} paid orders. Starting bulk send...`)
    ...
    for (const record of orders) {
      ...
      const isPUPian = ticket_type_id === PUP_STUDENT_ID
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
    ...
    <tr><td style="padding:24px 0 0;text-align:center;">
      ...
      <table cellpadding="0" cellspacing="0" style="margin:0 auto 12px;">
        <tr>
          <td style="padding:0 6px;"><a href="https://www.facebook.com/pupcommsoc" style="display:inline-block;width:32px;height:32px;background:#1a2340;border-radius:50%;text-align:center;line-height:32px;text-decoration:none;font-size:14px;color:#FAF5E9;">f</a></td>
          <td style="padding:0 6px;"><a href="https://www.instagram.com/pupcommsoc_/" style="display:inline-block;width:32px;height:32px;background:#1a2340;border-radius:50%;text-align:center;line-height:32px;text-decoration:none;font-size:14px;color:#FAF5E9;">ig</a></td>
          <td style="padding:0 6px;"><a href="https://x.com/pupcommsoc_" style="display:inline-block;width:32px;height:32px;background:#1a2340;border-radius:50%;text-align:center;line-height:32px;text-decoration:none;font-size:14px;color:#FAF5E9;">x</a></td>
          <td style="padding:0 6px;"><a href="http://tiktok.com/@pup_commsoc" style="display:inline-block;width:32px;height:32px;background:#1a2340;border-radius:50%;text-align:center;line-height:32px;text-decoration:none;font-size:14px;color:#FAF5E9;">tt</a></td>
        </tr>
      </table>
      ...
      try {
        ...
        if (!res.ok) {
          console.error(`FAILED: ${email} | ${ticket_code} | ${JSON.stringify(data)}`)
          results.failed++
          results.errors.push(`${email} (${ticket_code}): ${data.message}`)
        } else {
          console.log(`SENT: ${email} | ${ticket_code}`)
          results.success++
        }
      } catch (err) {
      ...
      // 250ms delay = ~4 emails/sec, safely under the 5 req/sec limit
      await delay(250)
    }
